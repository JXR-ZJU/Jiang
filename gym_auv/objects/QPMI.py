...
from scipy.optimize import fminbound  #  用于寻找函数的最小值
from mpl_toolkits.mplot3d import Axes3D  #  用于在三维空间中绘制图形。

#  实现一个二次插值路径规划器   Quadratic Path Motion Interpolation，QPMI）

class QPMI(): # 二次插值法
    def __init__(self, waypoints):
        #初始化了路径规划器，接收航路点（waypoints）作为输入，并计算了路径的参数。
    ...
    def _calculate_us(self):
        #  计算了每个航路点之间的距离，并存储在 us 属性中
        diff = np.diff(self.waypoints, axis=0) # 计算每两项之间的差值
        seg_lengths = np.cumsum(np.sqrt(np.sum(diff**2, axis=1))) 
        return np.array([0,*seg_lengths[:]]) # 从原点开始到每一个航路点的距离
    
    def calculate_quadratic_params(self):
        # 计算了二次插值所需的参数，分别对 x、y、z 方向进行计算。
        self.x_params = []
        self.y_params = []
        self.z_params = []
        for n in range(1, len(self.waypoints)-1):
            ...
            self.x_params.append(x_params_n) # 总共len(self.waypoints)-2个元素
    ...
    def calculate_ur(self, u):
        ...
        ur = (u-self.us[n])/(self.us[n+1] - self.us[n]) # n到n+1段内走过的比例
        return ur
    ...
    def __call__(self, u): # 每一段内ax、ay、az、bx、by、bz、cx、cy、cz一定，只有u不一样
        if u >= self.us[0] and u <= self.us[1]: # first stretch
            ...
            n = self.get_u_index(u)
            ur = self.calculate_ur(u) # ur和uf也与u有关
    ...
    def plot_path(self, wps_on=True):
        u = np.linspace(self.us[0], self.us[-1], 10000) # 从第一个点到最后一个点分成10000份
        quadratic_path = []
        for du in u:
                quadratic_path.append(self(du)) # 路径上等距离的点的坐标
                self.get_direction_angles(du) # 每个点前进方向
...
if __name__ == "__main__":
    ...
    # wps = np.array([np.array([0, 0, 0]), np.array([10, 5, 8]), np.array([25, 10, 10]), np.array([40, 10, 20]),
    #                 np.array([45, 25, 25]),
    #                 np.array([40, 40, 30]), np.array([25, 40, 20]), np.array([20, 60, 15]), np.array([20, 40, 10]),
    #                 np.array([0, 0, 0])])
    path = QPMI(wps)
