...
from stable_baselines3.common.on_policy_algorithm import OnPolicyAlgorithm3
from stable_baselines3.common.policies import ActorModelCriticPolicy
from stable_baselines3.common.type_aliases import GymEnv, MaybeCallback, Schedule
from stable_baselines3.common.utils import explained_variance, get_schedule_fn


class PPO3(OnPolicyAlgorithm3):
    """
    ...
    and Stable Baselines (PPO3 from https://github.com/hill-a/stable-baselines)
    ...
    def _setup_model(self) -> None:
        super(PPO3, self)._setup_model() # 初始化rolloutbuffer和policy
    ...
    def train(self) -> None:
        ...
        # 在on_policy_algorithm中定义，所以AUV共用一个policy
        ...
        for epoch in range(self.n_epochs): # 训练四轮，每次从rollout_buffer中采集batch_size个样本依次进行训练
            ...
            for i in range(len(self.rollout_buffer)):
                if i == 0:
                    rollout_datas = self.rollout_buffer[i].get(self.batch_size) # rollout_datas有1024/64=16个元素
                else:
                    rollout_datas = itertools.chain(rollout_datas, self.rollout_buffer[i].get(self.batch_size))
                # for rollout_data in self.rollout_buffer[i].get(self.batch_size): # 从数据库（n_steps*n_envs）随机获取batch_size个数据进行遍历
            for rollout_data in rollout_datas: # 每个rollout_data有64个
        ...
        # all_returns = []
        explained_var = []
        for i in range(self.n_AUVs):
            # all_values.extend(self.rollout_buffer[i].values)
            # all_returns.extend(self.rollout_buffer[i].returns)
            explained_var.append(explained_variance(self.rollout_buffer[i].values.flatten(), self.rollout_buffer[i].returns.flatten())) # 计算价值解释的关于回报的方差分数
        ...
        self.logger.record("train/loss", loss.item()) # 一轮更新完后的损失函数
        self.logger.record("train/explained_variance", explained_var) # 每个AUV价值解释的关于回报的方差分数
    ...
    def learn(
        ...
        tb_log_name: str = "PPO3",
        eval_log_path: Optional[str] = None,
        reset_num_timesteps: bool = True,
    ) -> "PPO3":

        return super(PPO3, self).learn(
